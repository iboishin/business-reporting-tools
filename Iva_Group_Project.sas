libname group 'C:\Users\fgalicojustitz\Documents\GitHub\BusinessReportingTools\Group_assignment';

*import databases;

DATA group.airlines;
	INFILE 'C:\Users\fgalicojustitz\Documents\GitHub\BusinessReportingTools\Group_assignment\airlines.csv' DLM=',' DSD FIRSTOBS=2;
	INPUT obs carrier $ airline_name :$27.;
run;

DATA group.airports;
	INFILE 'C:\Users\fgalicojustitz\Documents\GitHub\BusinessReportingTools\Group_assignment\airports.csv' DLM=',' DSD FIRSTOBS=2;
	INPUT obs faa $ name :$60. lat lon alt tz dst $ tzone :$20.;
run;

DATA group.flights;
	INFILE 'C:\Users\fgalicojustitz\Documents\GitHub\BusinessReportingTools\Group_assignment\flights.csv' DLM=',' DSD FIRSTOBS=2;
...
DATA group.planes;
	INFILE 'C:\Users\fgalicojustitz\Documents\GitHub\BusinessReportingTools\Group_assignment\planes.csv' DLM=',' DSD FIRSTOBS=2;
...
DATA group.weather;
	INFILE 'C:\Users\fgalicojustitz\Documents\GitHub\BusinessReportingTools\Group_assignment\weather.csv' DLM=',' DSD FIRSTOBS=2;
	INPUT obs origin $ year month day hour temp dewp humid wind_dir wind_speed wind_gust precip pressure visib time_hour $19.;
run;

*Create basetable merging all tables with flights table, with left join;
PROC SQL;
...
CREATE TABLE group.Airlines_delay as
	SELECT carrier, airline_name as Airline, ROUND(AVG(dep_delay),0.1) as Avg_Dep_Delay, ROUND(AVG(arr_delay),0.1) as Avg_Arr_Delay,
...
CREATE TABLE group.Origin_delay as
	SELECT origin, ROUND(AVG(dep_delay),0.1) as Avg_Dep_Delay, ROUND(MAX(dep_delay),0.1) as Max_Dep_Delay,
		   ROUND(MIN(dep_delay),0.1) as Min_Dep_Delay, ROUND(AVG(distance),0.1) as Avg_Distance, 
...
CREATE TABLE group.Dest_delay as
	SELECT dest, ROUND(AVG(arr_delay),0.1) as Avg_Arr_Delay, ROUND(MAX(arr_delay),0.1) as Max_Arr_Delay,
		   ROUND(MIN(arr_delay),0.1) as Min_Arr_Delay, ROUND(AVG(distance),0.1) as Avg_Distance, 
		   ROUND(AVG(air_time),0.1) as Avg_Air_Time, count(*) as Nr_Flights
	FROM group.Flights
	GROUP BY dest;
QUIT;
